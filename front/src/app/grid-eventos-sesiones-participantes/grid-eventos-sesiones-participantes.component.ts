...
import { GridEventosSesionesParticipantesService } from './grid-eventos-sesiones-participantes.service';
import { SnackbarService } from '../snackbar/snackbar.service';

type Registro = EventosSesionesParticipantes;
...
@Component({
  selector: 'app-grid-eventos-sesiones-participantes',
  standalone: false,
  templateUrl: './grid-eventos-sesiones-participantes.component.html',
  styleUrl: '../css/styles.css',
})
export class GridEventosSesionesParticipantesComponent {
  ...
  constructor(
    private gridEventosSesionesParticipantesService: GridEventosSesionesParticipantesService,
  ...
  cargarListaBeneficiarios() {
    ...
    this.gridEventosSesionesParticipantesService.cargarListaBeneficiarios().subscribe(
      (response) => {
  ...
  cargarRegistros() {
    ...
    //console.log("Llega la maestra del calendario:",this.maestraId);
    this.gridEventosSesionesParticipantesService
  ...
  async cancelarEdicion(registro: Registro | null): Promise<boolean> {
    ...
    if (cancelo) {
      ...
      if (registro !== null && registro.id === 0) {
        //console.log('deshaciendo el registro: ',registro.id_participante);
        this.registrosNuevos = this.registrosNuevos.filter(
          (p) => !(p.id === registro.id && p.id_participante === registro.id_participante)
        );
  ...
  async ordenarTabla(columna: string): Promise<void> {
    ...
    if (cancelo) {
      ...
      this.registros.sort((a, b) => {
        //console.log("Esta ordenando a:",a.id_participante, " vs ",b.id_participante)
  ...
  adicionarNuevaFila(i: number): void {
    const nuevoRegistro: Registro = {
      id: 0,
      evento_sesiones_id: this.maestraId,
  ...
  async eliminarRegistro(registro: Registro): Promise<void> {
    ...
    if (eliminar) {
      ...
      //console.log(    'antes de filtrar:',    registro.id_participante,    ' En eliminados: ',  this.registrosFiltrados);
      //Developer: Se debe buscar el registro en el arreglo de nuevos, usando lo equivalente a la llave única (no la llave primaria)
      this.registros = this.registros.filter(
        (p) => !(p.id === registro.id && p.id_participante === registro.id_participante)
      );
  ...
  @Output() eventosSesionesParticipantesEmitidos = new EventEmitter<{
  ...
  emitirRegistros() {
    console.log("Emitiendo registros...", this.registrosNuevos, this.registrosModificados, this.registrosEliminados);
    this.eventosSesionesParticipantesEmitidos.emit({
...
/*
  emitirRegistros() {
    // Convertir id_participante a number en todos los arreglos
    const nuevos = this.registrosNuevos.map((r) => ({
      ...r,
      id_participante: r.id_participante != null ? parseInt(r.id_participante as any, 10) : null,
    }));
    const modificados = this.registrosModificados.map((r) => ({
      ...r,
      id_participante: r.id_participante != null ? parseInt(r.id_participante as any, 10) : null,
    }));
    const eliminados = this.registrosEliminados.map((r) => ({
      ...r,
      id_participante: r.id_participante != null ? parseInt(r.id_participante as any, 10) : null,
    }));

    console.log('Emitiendo registros...', nuevos, modificados, eliminados);
    this.eventosSesionesParticipantesEmitidos.emit({
      nuevos,
      modificados,
      eliminados,
  ...
  actualizarNuevosId(nuevosIds: { id: number; id_participante: string }[]) {
    ...
    nuevosIds.forEach((nuevoId) => {
      ...
      this.registrosPaginados.forEach((registro) => {
        ...
        if (registro.id === 0 && registro.id_participante === nuevoId.id_participante) {
    ...
    //console.log('Registros actualizados - registrosPaginados:', this.registrosPaginados );
